...
class InterfaceScorer extends ClassScorer {
	...
	void evaluateScore(ClassNode node1, ClassNode node2) {
		...
		if(node1.interfaces.size() == 0 && node2.interfaces.size() == 0) {
			...
			if(matches > 0) {
				score = (ComparisonClassRemapper.SCORE_MAX * matches) / node1.interfaces.size();
			}
			else {
				int gameifaces1 = 0, gameifaces2 = 0;
				for(Object iface1: node1.interfaces) {
					if(!ComparisonClassRemapper.isStandardLibraryClass(classLoader,(String)iface1)) gameifaces1++;
				}
				for(Object iface2: node2.interfaces) {
					if(!ComparisonClassRemapper.isStandardLibraryClass(classLoader,(String)iface2)) gameifaces2++;
				}
				...
				if(denom != 0) {
					score = (numer * ComparisonClassRemapper.SCORE_MAX) / denom;
				}
				else {
					score = ComparisonClassRemapper.SCORE_UNAVAILABLE;
				}
			...
			if(matches > 0) {
				score = (ComparisonClassRemapper.SCORE_MAX * matches) / more.interfaces.size();
			}
			else {
				int gameifaces1 = 0, gameifaces2 = 0;
				for(Object iface1: node1.interfaces) {
